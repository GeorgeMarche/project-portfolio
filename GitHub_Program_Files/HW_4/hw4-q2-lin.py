...
try:
    ...
    # Get information on followers of @weiglemc
    c = tweepy.Cursor(api.followers_ids, screen_name = 'weiglemc', count = 200)
    n_followers = 0
    f_counts = []

    # For each follower, get their number of followers, increment count
    for f_id in c.items():
        n_followers += 1
    ...
    print("Weigle users: %d" % api.get_user('weiglemc').followers_count)
    ...
    f_data = {'label': f_labels, 'follower_count': f_counts}
    ...
    f_bar.set(title = "How Many Followers Do Dr. Weigle's Followers Have?",
            xlabel = "Follower",
    ...
    f_bar.savefig("LinFollowerCount.png")
